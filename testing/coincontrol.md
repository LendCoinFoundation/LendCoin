...
This report covers test results of the coin control feature of LendCoin v0.4.0 released [here](http://www.LendCointalk.org/index.php?topic=2648.msg23426#msg23426). The tests are done on windows 7 professional 32bit. LendCoin distribution LendCoin-rc.zip is downloaded and unzip'ped in D:\bin

## Basic features

The test method used is basically clicking/typing on the GUI of LendCoin running on LendCoin testnet, and comparing the results with those specified in [_Yet another Coin Control Release_ by cozz](https://bitcointalk.org/index.php?topic=144331.0).
...
```
D:\bin\LendCoin-rc\LendCoin-rc-qt -conf=D:\bin\LendCoin-rc\LendCoin.conf
```
The config file D:\bin\LendCoin-rc\LendCoin.conf contains
```
datadir=D:\bin\LendCoin-rc
testnet=1
server=1
rpcuser=mh
rpcpassword=x
...
* After LendCoin-rc is started, in the debug->Information the "On testnet" box is in checked as expected. 
* peerunit-rc successfully syncs with the testnet. "D:\bin\LendCoin-rc\testnet" subdirectory is created with downloaded block chain.
...
* These labels are tested **except that priorities are not verified because I do not know how priority levels for LendCoins and Peershares are defined**. 
...
* LendCoin has a fixed transaction fee so this part below about free transaction calculation is irrelevant hence not tested. 
...
## Summary
The coin control implmented in LendCoin-rc passed tests of all main function items [specified by cozz](https://bitcointalk.org/index.php?topic=144331.0). One minor item that did not pass, and two minor untested items, are marked in bold above.

## History
Test reprt for an early build of LendCoin_coin-control is [here](http://www.LendCointalk.org/index.php?topic=2699.msg23386#msg23386).
